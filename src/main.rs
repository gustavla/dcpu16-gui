...
const MONITOR_WIDTH: u32 = COLS as u32 * FONT_WIDTH as u32;
const MONITOR_HEIGHT: u32 = ROWS as u32 * FONT_HEIGHT as u32;
const SCALE: u32 = 5;
const BORDER: u32 = 5*SCALE;
...
impl GUIHWMonitorLEM1802 {
    ...
    pub fn pre_connect(self, location: u16) -> GUIHWMonitorLEM1802 {
    ...
    pub fn font_location(self, location: u16) -> GUIHWMonitorLEM1802 {
        let mut new_self = self;
        new_self.font_location = Some(location);
        new_self
    }

    pub fn palette_location(self, location: u16) -> GUIHWMonitorLEM1802 {
        let mut new_self = self;
        new_self.palette_location = Some(location);
        new_self
    }

    pub fn border_color_index(self, border_color_index: u16) -> GUIHWMonitorLEM1802 {
        let mut new_self = self;
    ...
    fn get_data(&self, cpu: &DCPU, blinkout: bool) -> Vec<u8> {
...
fn main() {
    ...
    let mut monitor = GUIHWMonitorLEM1802::new();
    // Attach monitor
    //if pre_connected {
        //monitor = monitor.pre_connect(0x8000);
    //}
        ...
        Some(s) => {
            if s.len() >= 2 && s.chars().nth(0).unwrap() == '0' && (s.chars().nth(1).unwrap() == 'X' || s.chars().nth(1).unwrap() == 'x') {
                match isize::from_str_radix(&s[2..], 16) {
                    Ok(loc) => monitor.pre_connect(loc as u16),
                    Err(e) => {
    ...
    monitor = match matches.opt_str("font") {
        Some(s) => {
            if s.len() >= 2 && s.chars().nth(0).unwrap() == '0' && (s.chars().nth(1).unwrap() == 'X' || s.chars().nth(1).unwrap() == 'x') {
                match isize::from_str_radix(&s[2..], 16) {
                    Ok(loc) => monitor.font_location(loc as u16),
                    Err(e) => {
    ...
    cpu.devices.push(Box::new(monitor));
    ...
    while let Some(e) = events.next(&mut window) {
        ...
        if let Some(_) = e.render_args() {
            ...
            cpu.run(3000);
            let dev = cpu.devices.get(0).unwrap();
            if let Some(monitor) = dev.as_any().downcast_ref::<GUIHWMonitorLEM1802>() {
                let v = &monitor.get_data(&cpu, blinkout)[..];
                for k in 0..(v.len() / 3) {
